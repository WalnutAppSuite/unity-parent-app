...
const ObservationAccordion = (data: CustomAccordionProps) => {
    ...
    return (
        <Accordion
            variant="separated"
            value={isOpen ? "english" : null}
            onChange={(value) => setIsOpen(value === "english")}
            style={{
        ...
        >
            <Accordion.Item value="english" style={{ border: "none" }}>
                <Accordion.Control 
                    style={{ 
                        backgroundColor: "#ffffff", 
                        padding: "18px",
                ...
                >
                    <Group position="apart" style={{ width: "100%" }}>
                        ...
                        {/* Secondary elements: badges */}
                        {!isOpen && (
                            <Group spacing={10}>
                                {observations.map((observation) => {
                                    // More nuanced color calculation for better visual hierarchy
                                    const scorePercentage = (observation.division_average / observation.total_marks) * 100;
                                    ...
                                    const bgColor =
                                        scorePercentage >= 80 ? "#f0fdf4" :
                                        scorePercentage >= 60 ? "#f7fee7" :
                                        scorePercentage >= 40 ? "#fef3c7" :
                                        scorePercentage >= 20 ? "#fee2e2" : 
                                        "#fecaca";
