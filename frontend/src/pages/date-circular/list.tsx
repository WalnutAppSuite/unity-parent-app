...
const CmapDateList = () => {
  ...
  const studentName = studentsList?.data?.message?.find(
  ...
  return (
    <Box>
      ...
      {Object.keys(cmapList?.data?.message).map((subject) => {
        return Object.keys(cmapList?.data?.message?.[subject]).map((unit) => (
          <>
            <Box
              sx={{
                marginBottom: 10,
            ...
            >
              {!cmapList?.data?.message?.[subject]?.[unit]?.length && (
                <Text align="center" color="dimmed" weight="bold" my={30}>
                  {isLoading ? "Loading..." : "No Curriculum Found"}
                </Text>
              )}
              {cmapList?.data?.message?.[subject]?.[unit]?.map((cmap, i) => {
                const isOpen = openedCmap === i.toString();
                const openOrClose = () =>
                  isOpen ? setOpenedCmap("") : setOpenedCmap(i.toString());
                const broadCasts = Array.from(
                  new Set(
                    cmap?.products?.map(
                      (product) => product?.broadcast_description
                    )
                  )
                ).filter(Boolean);
                const homeWorks = Array.from(
                  new Set(
                    cmap?.products?.map(
                      (product) => product?.homework_description
                    )
                  )
                ).filter(Boolean);
                const parentNotes = Array.from(
                  new Set(
                    cmap?.products?.map(
                      (product) => product?.parentnote_description
                    )
                  )
                ).filter(Boolean);
                const products = cmap?.products?.filter(
                  (product) => product?.item_data?.custom_product_url
                );
                const cmapTitle = Array.from(
                  new Set(cmap?.products?.map((product) => product?.chapter))
                )
                  .filter(Boolean)
                  .join(", ");
                return (
                    ...
                    >
                      ...
                      >
                        {products.map((product, i) => {
                          ...
                          const fileType =
                            fileTypes[
                              product?.item_group as keyof typeof fileTypes
                            ] || fileTypes.default;
                          return (
                            <Box
                              key={i}
                            ...
                            >
                              {isOpen && (
                                <Box
                                  component="a"
                                  href={
                                    product?.item_data?.custom_product_url ||
                                    "#"
                                  }
                                  target="_blank"
                                  onClick={(e) => {
