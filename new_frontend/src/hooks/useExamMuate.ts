...
export function useFetchAssessmentGroups(
    ...
    setPrintFormat: (format: { html: string; style: string }) => void,
    ...
    const assessmentGroupFilter = async (
        ...
        const resp = await fetch(
            `/api/resource/Assessment%20Group?filters=[["custom_academic_year","=","${selected_year}"],["custom_program","=","${class_name}"],["custom_show_in_app","=", "1"]]&fields=["assessment_group_name","name"]`
        );
        if (!resp.ok) {
